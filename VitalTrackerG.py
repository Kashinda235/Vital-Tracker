...
# Prevents the scroll bar jump by locking container height
st.markdown("""
    <style>
...
def main():
    ...
    while run_monitoring:
        ...
        with chart_placeholder.container():
            ...
            # Heart Rate
            fig.add_trace(go.Scatter(x=hist['Timestamp'], y=hist['Heart Rate'][-11:-1], name="BPM", line=dict(color='red')), row=1, col=1)
            # BP
            fig.add_trace(go.Scatter(x=hist['Timestamp'], y=hist['Systolic'][-11:-1], name="Sys", line=dict(color='blue')), row=1, col=2)
            fig.add_trace(go.Scatter(x=hist['Timestamp'], y=hist['Diastolic'][-11:-1], name="Dia", line=dict(color='green')), row=1, col=2)
            # Temp
            fig.add_trace(go.Scatter(x=hist['Timestamp'], y=hist['Temperature'][-11:-1], name="Temp", line=dict(color='orange')), row=2, col=1)
            # SpO2
            fig.add_trace(go.Scatter(x=hist['Timestamp'], y=hist['Oxygen'][-11:-1], name="SpO2", line=dict(color='teal')), row=2, col=2)
